...
var suit_tracks : Array[SuitTrackGUI]

var lock : Lock = Lock.new()

func _ready():
	for suit in debate_settings.suits:
		var suit_track : SuitTrackGUI = suit_track_packed_scene.instantiate()
	...
	for i in amount:
		...
		for track in suit_tracks:
			if debate_settings.line_clear_direction == Const.Direction.RIGHT:
				remove_funcs.append(func(): await track.remove_token_at(track.tokens_gui.size() - 1))
			else:
...
func update_board(suit_track_dictionary : Dictionary, active_contestant : String):
	...
	for track : SuitTrackGUI in suit_tracks:
		var suit = track.suit.name
	
		var current_tokens = track.tokens_gui.map(func(x): return x.token)
