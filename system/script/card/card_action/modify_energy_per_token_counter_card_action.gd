...
class_name ModifyEnergyPerTokenCounterCardAction
...
func invoke(caller : Card, player : Contestant, manager : DebateManager) -> bool:
	...
	contestant.current_energy = EnumMath.evaluate(contestant.current_energy, amount * caller.token_counter, operation)
